...
function Exam02Auth(props) {
  ...
  const dispatch = useDispatch(); //dispatch가져오기
  ...
  const handleLogin = async (event) => {
    try {
      //로그인 요청
      const userInfo = {...user}; //복제해서 사용하기
      const response = await login(userInfo);
      console.log(response);
      //요청 헤더에 JWT 토큰 추가
      addAuthHeader(response.data.authToken); //response로 받아온 토큰을 저장. 다음 axios 요청할 때 authToken을 header에 심어서 보내기 위함
      //Redux에 인증 내용 저장. userid와 authToken을 저장
